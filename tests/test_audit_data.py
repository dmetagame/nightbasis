...
class AuditDataTests(unittest.TestCase):
    ...
    def test_juneteenth_is_not_a_target_session(self):
        ...
        windows = session_windows(dt.date(2026, 6, 17), dt.date(2026, 6, 23), now)
        kinds = {item[0]: item[3] for item in windows}
        self.assertEqual(kinds[dt.date(2026, 6, 19)], "weekend_or_holiday")
        self.assertIn(dt.date(2026, 6, 22), kinds)
    ...
    def test_session_uses_frozen_1615_to_0900_window(self):
        ...
        target, left, right, kind = windows[0]
        ...
        self.assertEqual(kind, "weeknight")
